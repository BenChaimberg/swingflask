...
from static import producttitle, producttext, productdir, productinfo, productdemo, productforms, frenchproducttitle, frenchproducttext, frenchproductdir, frenchproductinfo, frenchproductdemo, frenchproductforms, categorytitle, categoryproducts, frenchcategorytitle, frenchcategoryproducts
...
def add_products_sql():
	for product in producttitle.title:
		for item in categoryproducts.products: #for every category in categories
			for item2 in categoryproducts.products[item]: #for every product in category
				...
				if (product == '1818' or product == '1819') and item2 == '1817': #if productid is 1818 or 1819, not in list, so uses 1817 instead
					category = item #return category
		if productdemo.demo[product] is not 'No Directions Available':
			demo = productdemo.demo[product]
		else: demo = ''
		new_product = Products(product,
			producttitle.title[product],
			demo,
			producttext.text[product],
			productdir.dir[product],
			productforms.forms[product+'us'],
			productforms.forms[product+'can'],
			category
		)
		db.session.add(new_product)
		for i in range(0,len(productinfo.info[product])/2):
			info = productinfo.info[product].pop(0)
			for j in range(0,len(productinfo.info[product])/2+1):
				info2 = productinfo.info[product].pop(0)
				new_infotable = Infotable(product,info,info2)
				break
			db.session.add(new_infotable)
		for info in productinfo.info2[product]:
			new_infolist = Infolist(product,info)
			db.session.add(new_infolist)
	db.session.commit()

def add_category_sql():
	for category in categorytitle.title:
		new_category = Categories(category,categorytitle.title[category])
